...
- [**计概 CHEAT-SHEET**](#计概-cheat-sheet)
    ...
    - [BASIC GRAMMAR](#basic-grammar)
      ...
      - [6. euler\_sieve](#6-euler_sieve)
      - [7. quick power](#7-quick-power)
      - [8. pisano period](#8-pisano-period)
    - [并查集](#并查集)
      - [1. 统计连通分量数量](#1-统计连通分量数量)
      - [2. 获取所有连通分量](#2-获取所有连通分量)
      - [实际应用示例](#实际应用示例)
        - [例题1：朋友圈问题](#例题1朋友圈问题)
        - [例题2：岛屿数量](#例题2岛屿数量)
    - [树状数组](#树状数组)
...
```python
#运算符相关--波兰表达式
def infix_to_postfix(infix): #中缀变后缀
...
def postfix_eval(postfix): #运算后缀表达式
    stack = []
...
---
#### 2.QUEUE
队列(queue)，使用`list`来模拟，遵循先进先出的原则。
例题：
```python
#oj-队列-22066
from collections import deque
...
def update_point(self, i, val):
    self.update(i, val - self.query(i - 1))
```
#### 4. 区间最大值
```python
def query_max(self, l, r):
    return max(self.query(l), self.query(r) - self.query(l - 1))
```
#### 5. 区间最小值
```python
def query_min(self, l, r):
    return min(self.query(l), self.query(r) - self.query(l - 1))
```
#### 6. 区间第k小值
```python
def query_kth(self, l, r, k):
    ...
    mid = (l + r) // 2
    if self.query(mid) >= k:
        return self.query_kth(l, mid, k)
    else:
        return self.query_kth(mid + 1, r, k - self.query(mid))
```
